...
def train_mnist_with_sparsity(gpu, batchsize, out, epoch, frequency, plot,
                              resume, target_layers, target_batchnorm_layers, lasso_rate, train):
    ...
    optimizer.add_hook(Lasso(lasso_rate, target_batchnorm_layers=target_batchnorm_layers))
...
def train_mnist_fine_tuning(model, gpu, batchsize, out, epoch, frequency, plot, resume):
    print('GPU: {}'.format(gpu))
    print('# Minibatch-size: {}'.format(batchsize))
    print('# epoch: {}'.format(epoch))
    print('')
...
def main():
    ...
    # for channel pruning
    parser.add_argument('--rate', type=float, default=1e-2,
                        help='Coefficient for the weight decay for Network Slimming')
    ...
    target_batchnorm_layers = ['/bn1', '/bn2']

    model = train_mnist_with_sparsity(args.gpu, args.batchsize, args.out,
                                      args.epoch, args.frequency, args.plot, args.resume,
                                      target_layers, target_batchnorm_layers, args.rate, not args.skip_pretrain)

    x = chainer.Variable(np.zeros((1, 1, 28, 28), dtype=np.float32))
    if args.gpu >= 0:
        # Make a specified GPU current
        chainer.backends.cuda.get_device_from_id(args.gpu).use()
        x.to_gpu()  # Copy to the GPU
    ...
    if args.reinitialize:
        model.to_cpu()
        reinitialize_model(model.predictor)
        if args.gpu >= 0:
            # Make a specified GPU current
            chainer.backends.cuda.get_device_from_id(args.gpu).use()
            model.to_gpu()  # Copy to the GPU

    train_mnist_fine_tuning(model, args.gpu, args.batchsize, args.out,
                            args.epoch, args.frequency, args.plot, args.resume)
